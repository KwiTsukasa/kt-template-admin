  ...
  setup(props) {
    const label = (entry: RuleConditionTrace) => {
      ...
      const field = props.fields.find(
        (candidate) => candidate.key === entry.field,
      );
      return `${field?.label || entry.field} · ${operators[entry.operator || ''] || entry.operator}`;
    };
    const color = (matched: boolean) => {
      if (matched) return 'success';
      return 'default';
    };
    const result = (matched: boolean) => {
      if (matched) return '满足';
      return '不满足';
    };
    return () => (
      <section aria-label="逐条件解释" class="mt-4 space-y-2">
        <h3>逐条件解释</h3>
        {props.trace.map((entry) => (
          <div
          ...
          >
            ...
            <Tag color={color(entry.matched)}>{result(entry.matched)}</Tag>
          </div>
        ))}
      </section>
    );
