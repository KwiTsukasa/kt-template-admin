...
import { getPluginTaskRunPage } from '#/api/plugin-platform/task';
...
export default defineComponent({
  name: 'PluginPlatformTaskRunDrawer',
  props: {
    open: {
      default: false,
      type: Boolean,
  ...
  setup(props, { emit }) {
    ...
    const runs = ref<PluginPlatformTaskApi.TaskRun[]>([]);
    ...
    async function loadRuns() {
      ...
      loading.value = true;
      try {
        const page = await getPluginTaskRunPage(props.task.id, {
          pageNo: 1,
          pageSize: 20,
        });
    ...
    const renderRun = (item: PluginPlatformTaskApi.TaskRun) => (
