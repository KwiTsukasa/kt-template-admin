...
/**
 * 通过编排 Bot 新建或刷新登录的扫码会话，统一处理二维码、轮询、SSE 进度、安全验证和清理。
 *
...
export function useNapcatLoginSession(options: NapcatLoginSessionOptions) {
  ...
  const scanEvents = ref<BotNapcatApi.AccountScanEvent[]>([]);
  ...
  /**
   * 以新建模式启动 NapCat 扫码会话，供登录并创建新的 Bot 账号。
   */
   ...
   *
   * @param row - 要刷新登录状态的 Bot 账号记录。
   */
  async function openRefresh(row: BotApi.Account) {
   ...
   * @param row - 刷新模式下要重新建立登录态的现有 Bot 账号；创建模式可省略。
   */
  async function startScan(mode: 'create' | 'refresh', row?: BotApi.Account) {
    resetScanState(mode);
    options.openModal(scanTitle.value);
    scanLoading.value = true;
    try {
      if (mode === 'create') {
        await applyScanResult(await startBotAccountScanCreate(), {
          reloadQrcode: true,
        });
        return;
      }
      ...
      await applyScanResult(await startBotAccountScanRefresh(row.id), {
        reloadQrcode: true,
      });
    } catch (error) {
      stopScanPolling();
  ...
  async function pollScanStatus() {
    ...
    scanLoading.value = true;
    try {
      await applyScanResult(await getBotAccountScanStatus(scanState.sessionId));
    } finally {
      scanLoading.value = false;
    }
  }
  ...
  async function refreshScanQrcode() {
    ...
    try {
      await applyScanResult(
        await refreshBotAccountScanQrcode(scanState.sessionId),
        { reloadQrcode: true },
      );
    } finally {
      scanLoading.value = false;
  ...
  async function submitScanCaptcha() {
    ...
    try {
      ...
      await applyScanResult(
        await submitBotAccountScanCaptcha({
          ...captcha,
          sessionId,
        }),
      );
  ...
  function startScanEvents(sessionId?: string) {
    ...
    const source = new EventSource(getBotAccountScanEventsUrl(sessionId), {
  ...
  function handleScanEvent(payload: string) {
    try {
      const event = JSON.parse(payload) as BotNapcatApi.AccountScanEvent;
  ...
  function cleanupScanSession() {
    ...
    if (sessionId && scanState.status === 'pending') {
      void cancelBotAccountScan(sessionId);
    }
  }
