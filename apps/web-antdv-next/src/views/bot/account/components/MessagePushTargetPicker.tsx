...
type TargetType = BotMessageSubscriberApi.TargetType;
...
export interface MessagePushTargetPickerProps {
  ...
  loading?: boolean;
  manualEntry: boolean;
  options: BotMessageSubscriberApi.TargetOption[];
  reasonCode: null | string;
  value: BotMessageSubscriberApi.PublishTargetInput[];
}
...
export function isValidMessagePushTargetId(
  targetId: string,
  connectionMode: BotApi.ConnectionMode | null = 'reverse-ws',
): boolean {
...
export default defineComponent({
  ...
  props: {
    ...
    connectionMode: {
      default: null,
      type: String as PropType<BotApi.ConnectionMode | null>,
    },
    disabled: {
      default: false,
      type: Boolean,
    ...
    options: {
      required: true,
      type: Array as PropType<BotMessageSubscriberApi.TargetOption[]>,
    },
    reasonCode: {
      default: null,
    ...
    value: {
      default: () => [],
      type: Array as PropType<BotMessageSubscriberApi.PublishTargetInput[]>,
    },
  },
  emits: {
    'update:value': (value: BotMessageSubscriberApi.PublishTargetInput[]) =>
      Array.isArray(value),
  },
  setup(props, { emit }) {
    ...
    return () => (
      <div
        class="bot-message-push-target-picker w-full"
        style={{ width: '100%' }}
      >
        {(() => {
          if (props.available) {
...
function mergeTargets(
  current: BotMessageSubscriberApi.PublishTargetInput[],
  options: BotMessageSubscriberApi.TargetOption[],
  changedType: TargetType,
  changedIds: string[],
): BotMessageSubscriberApi.PublishTargetInput[] {
