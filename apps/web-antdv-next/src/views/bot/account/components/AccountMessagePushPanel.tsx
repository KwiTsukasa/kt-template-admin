...
type AccountMessagePushBodyCellSlot = {
  column: TableColumnType<BotMessageSubscriberApi.PublishBindingView>;
  record: BotMessageSubscriberApi.PublishBindingView;
};

const PERMISSIONS = {
  create: 'Bot:Account:MessagePush:Create',
  delete: 'Bot:Account:MessagePush:Delete',
  list: 'Bot:Account:MessagePush:List',
  toggle: 'Bot:Account:MessagePush:Toggle',
  update: 'Bot:Account:MessagePush:Update',
} as const;
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const targetOptions = ref<BotMessageSubscriberApi.TargetOptionsResponse>();
    const targetOptionsLoading = ref(false);
    let loadRevision = 0;
    let latestBindingPage: {
      items: BotMessageSubscriberApi.PublishBindingView[];
      total: number;
    } = { items: [], total: 0 };

    const columns: Array<
      TableColumnType<BotMessageSubscriberApi.PublishBindingView>
    > = [
      {
    ...
    const api: KtTableApi<BotMessageSubscriberApi.PublishBindingView> = {
      list: async () => {
        const revision = loadRevision;
        const selfId = props.selfId;
        const bindings = await getBotMessageBindings(selfId);
    ...
    const buttons: Array<
      KtTableButton<BotMessageSubscriberApi.PublishBindingView>
    > = [
      {
    ...
    const rowActions: Array<
      KtTableRowAction<BotMessageSubscriberApi.PublishBindingView>
    > = [
      {
        key: 'edit',
        label: '编辑',
    ...
    const [registerTable, tableApi] =
      useKtTable<BotMessageSubscriberApi.PublishBindingView>({
        api,
        buttons,
        columns,
        immediate: false,
    ...
    function getDeleteConfirm(
      row: BotMessageSubscriberApi.PublishBindingView,
    ): string {
    ...
    async function handleToggle(
      row: BotMessageSubscriberApi.PublishBindingView,
      context: KtTableContext<BotMessageSubscriberApi.PublishBindingView>,
    ) {
      await setBotMessageBindingEnabled(props.selfId, row.id, !row.enabled);
    ...
    async function handleDelete(
      ...
      await deleteBotMessageBinding(props.selfId, row.id);
      await context.reload();
    }
    ...
    /**
     * 加载指定账号可用的 Bot 订阅与目标，并忽略已经过期的响应代次。
     *
    ...
    async function loadMetadata(selfId: string, revision: number) {
      ...
      const [subscriptionResult, targetResult] = await Promise.allSettled([
        loadAllPages((params) =>
          getMessageSubscriptionList({ ...params, subscriberKey: 'bot' }),
        ),
        (() => {
          if (canLoadTargets) {
            return getBotMessageTargets(selfId);
          }
      ...
      if (subscriptionResult.status === 'fulfilled') {
        subscriptions.value = subscriptionResult.value.filter(
          (subscription) => subscription.subscriberKey === 'bot',
        );
      }
    ...
    return () => (
      <div class="bot-account-config-panel__spin bot-account-message-push-panel">
        {(() => {
          if (canList) {
            return (
              <>
                <AKtTable
                  class="bot-account-config-panel__table bot-account-message-push-panel__table"
