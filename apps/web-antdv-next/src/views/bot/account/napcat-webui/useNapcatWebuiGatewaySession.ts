...
export function useNapcatWebuiGatewaySession(accountId: Ref<string>) {
  const account = ref<BotNapcatApi.WebuiGatewaySessionAccount>();
  const container = ref<BotNapcatApi.WebuiGatewaySessionContainer>();
  ...
  async function open() {
    ...
    try {
      const result = await createBotNapcatWebuiSession({
  ...
  async function revoke() {
    ...
    try {
      await revokeBotNapcatWebuiSession(currentSessionId);
    } catch (error) {
      errorMessage.value = getErrorMessage(
        error,
  ...
  async function sendHeartbeat() {
    ...
    try {
      const result = await heartbeatBotNapcatWebuiSession(currentSessionId);
   ...
   */
  function applyGatewaySession(result: BotNapcatApi.WebuiGatewaySession) {
  ...
  async function revokeDetachedSession(staleSessionId: string) {
    try {
      await revokeBotNapcatWebuiSession(staleSessionId);
    } catch {
