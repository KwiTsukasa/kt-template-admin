import {
  cancelBotAccountScan,
  createBotNapcatWebuiSession,
  getBotAccountScanEventsUrl,
  getBotAccountScanStatus,
  getBotNapcatRuntimeDetail,
  ...
  refreshBotAccountScanQrcode,
  resolveNapcatLoginDisplayQrcode,
  revokeBotNapcatWebuiSession,
  startBotAccountScanCreate,
  startBotAccountScanRefresh,
  submitBotAccountScanCaptcha,
} from '@test-source/apps/web-antdv-next/src/api/bot/napcat';
...
describe('napcat login display helpers', () => {
  ...
  it('owns create, refresh, status, QR refresh, captcha, cancel, and SSE caller routes', async () => {
    ...
    await expect(startBotAccountScanCreate()).resolves.toBe(scanResult);
    await expect(startBotAccountScanRefresh('account-1')).resolves.toBe(
      scanResult,
    );
    await expect(getBotAccountScanStatus('scan-1')).resolves.toBe(scanResult);
    await expect(refreshBotAccountScanQrcode('scan-1')).resolves.toBe(
      scanResult,
    );
    await expect(
      submitBotAccountScanCaptcha({
    ...
    await expect(cancelBotAccountScan('scan-1')).resolves.toBe(scanResult);

    expect(requestClient.post).toHaveBeenCalledWith(
      '/bot-adapter/napcat/account/scan/create',
    );
    expect(requestClient.post).toHaveBeenCalledWith(
      '/bot-adapter/napcat/account/scan/refresh?id=account-1',
    );
    ...
    expect(requestClient.post).toHaveBeenCalledWith(
      '/bot-adapter/napcat/account/scan/qrcode/refresh?sessionId=scan-1',
    );
    expect(requestClient.post).toHaveBeenCalledWith(
    ...
    expect(requestClient.post).toHaveBeenCalledWith(
      '/bot-adapter/napcat/account/scan/cancel?sessionId=scan-1',
    );
    expect(getBotAccountScanEventsUrl('scan 1')).toBe(
      '/api/bot-adapter/napcat/account/scan/events?sessionId=scan%201',
    );
  });
  ...
  it('builds the read-only NapCat runtime detail request', async () => {
    ...
    await getBotNapcatRuntimeDetail('account-1');
  ...
  it('owns NapCat WebUI gateway session caller routes', async () => {
    const sessionResult = {
      ...
      iframeUrl: '/bot/napcat/webui/session/session-1/',
      sessionId: 'session-1',
    };
    ...
    await expect(
      createBotNapcatWebuiSession({ accountId: 'account-1' }),
    ).resolves.toBe(sessionResult);
    await expect(heartbeatBotNapcatWebuiSession('session-1')).resolves.toBe(
      lifecycleResult,
    );
    await expect(revokeBotNapcatWebuiSession('session-1')).resolves.toEqual({
      sessionId: 'session-1',
      status: 'revoked',
    });
    ...
    expect(requestClient.post).toHaveBeenCalledWith(
      '/bot-adapter/napcat/webui/session/session-1/heartbeat',
    );
    expect(requestClient.post).toHaveBeenCalledWith(
      '/bot-adapter/napcat/webui/session/session-1/revoke',
    );
  });
});
