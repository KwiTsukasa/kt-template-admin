...
} from '@test-source/apps/web-antdv-next/src/views/bot/account/components/MessagePushTargetPicker';
import Select from 'antdv-next/dist/select/index';
import { describe, expect, it } from 'vitest';

/** 创建群聊与私聊候选目标。 */
function createOptions(): BotMessageSubscriberApi.TargetOption[] {
  return [
    {
      label: '帕鲁测试群',
...
function latestValue(
  ...
  return emitted as BotMessageSubscriberApi.PublishTargetInput[];
}
...
describe('message-push target picker', () => {
  ...
  it('renders exactly two controlled tags Selects with type-scoped options', () => {
    ...
    expect(
      wrapper.get('.bot-message-push-target-picker').attributes('style'),
    ).toContain('width: 100%');
