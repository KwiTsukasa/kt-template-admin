...
- 顶级“大模型 / 大模型配置”只使用卡片看板；卡片底部操作栏只显示带 tooltip 的语义图标，不提供表格切换，也不在卡片中选择模型。新增与编辑抽屉只保存名称、供应商、端点、凭据及启停/默认状态，不出现“可用模型”手填项。本地 Codex 固定使用部署声明的私有 gateway 且不接收 API Key。进入统一流式对话页时调用 `/llm/configs/:id/models` 按供应商协议实时加载模型及其推理强度/速度能力，已有会话模型仍可用时保留，否则选择当前首项；只显示当前模型实际支持的能力控件，未声明的推理强度或速度档位保持隐藏。路由 Tag 跟随当前模型名称，内容区不重复显示固定“流式对话”标题；配置卡与会话内路由替换都使用 `llm-chat-<configId>` 作为同一稳定 `pageKey`，因此 `conversationId` 变化只更新当前 Tag，不会新增第二个对话 Tag，Tab Store 同时清理旧版遗留的同 name/path fullPath 页签且不触发导航。`LlmChat` 路由保持 `keepAlive`，普通页面切换不卸载或取消当前生成；`/api/llm/` 代理显式关闭缓冲和缓存。共享 Composer 为 2–8 行自动增长，IME 合成态与 Shift+Enter 不触发发送，计数、提示和发送/停止语义图标位于独立工具栏，生成中只把同一操作位切换为停止。收到的 SSE 文本先写入响应式消息，再以可排空的自适应字符队列持续呈现打字机效果，完成态必须等待已收内容全部显示；Assistant 卡片、Card body、Markdown、Milkdown 与 ProseMirror 使用同一背景变量，不能在气泡内部形成底色断层。发现失败或空目录时禁止发送，停止生成只中断当前流，不存在非流式回退。
- 共享对话页把路由 `conversationId` 作为持续状态输入，同一 config 的稳定 Tag 在会话间切换时以 latest-wins 重新载入，URL 与 DOM 不得错配；消息正文使用居中阅读宽度，历史阅读停止自动抢滚动并提供“回到最新”，Composer 初始 1 行、最多 6 行。失败或中断的空回复显示明确状态，不再保留“正在等待输出”的空卡片。
...
- 固定 `51825 → 51820` 的 WireGuard UDP 行例外显示并控制 `UDP NATMap`，不再误标成 Keeper 或静态路径；其他 UDP 行仍沿用 Keeper。两类 UDP 历史分别标记 `udp_natmap` 与 `udp_stun`，WireGuard endpoint 不借用 TCP IP4P/DDNS 语义。
- 顶级“媒体治理”默认进入系列资料库，目录固定为 `Series → Work → Season/Episode → Task`。TMDB TV Series 创建会原子带入官方正集数 Season/Episode；TV 零集空态只显示“尚未建立季集”，不能按电影/剧场版呈现。电影和剧场版保持无伪季独立 Work。全局 Task 页只读，来源、逐文件映射、下载、目录/文件名归一化和机械验收按 revision 执行。
- 治理 Task 不再显示元数据或 CodexAgent 页签，也没有 Agent 队列/会话/按钮。`governance.execute` 后只进入机械验收，关闭后不因 NFO、海报或 NAS 资料库关联失败而重开。
- `/media/scrape-validation` 是独立“NAS 刮削校验”页面，显示 `pending/running/healthy/issues`、缺项和基于校验自身 revision 的重新校验。该页面不调用任何治理 Task 写接口；运行中的记录隐藏重试，避免并发领取。
- 媒体任务 SSE 继续按 `runId/runSequence` 原位合并；只有序号缺口或 `snapshot-required` 静默回读快照。Series、Task、刮削校验和 LLM 配置看板继续复用 `KtCardList`/`KtTable` 的统一布局与语义图标操作栏。
