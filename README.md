...
- Admin 动态卡片列表统一由无业务依赖的 `KtCardList` 承担摘要 slot、空态、左对齐最大内容宽度、响应式网格和间距；只开放固定 `default/compact` 两种密度。系列资料库、媒体执行任务、大模型配置、系列详情季卡和 Episode 卡均已迁移，页面不得再手写 `auto-fill` 网格、平行空态或擅自居中限宽内容区。拓扑可视化、静态统计卡和聊天消息流不属于卡片列表。
