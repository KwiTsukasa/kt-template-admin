...
- 顶级“媒体治理”以“系列资料库”为默认入口，唯一层级为 `Series → Work → Season/Episode → Task`。新 Series 必须先搜索并选择官方主身份，创建时原子建立主 Work；TV、电影和剧场版都作为 Series 下的 Work，只有 TV Work 可创建 Season/Episode，电影与剧场版不伪造 S00。系列列表固定使用 KtTable 筛选/分页和卡片 footer，不提供表格/看板切换；卡片操作栏只显示带 Tooltip 与 `aria-label` 的语义图标。系列详情占满页面剩余高度，TV Work 的各季与电影/剧场版 Work 展平为同一排 Antdv card Tabs 上下文，不再堆叠 Work/Season 两排导航或自绘按钮卡；Episode 使用普通 KtTable 加服务端分页，并分别提供 RSS 订阅与执行历史 Tab。
