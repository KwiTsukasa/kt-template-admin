...
- Admin 动态卡片列表统一由无业务依赖的 `KtCardList` 承担摘要 slot、空态、全宽响应式网格和间距；`default/compact` 只规定最小可读密度，不规定卡片最大宽度或每排数量。系列资料库、媒体执行任务、大模型配置、系列详情季卡和 Episode 卡均已迁移，卡片以 `minmax(..., 1fr)` 随实时可用宽度自动伸缩；页面不得再手写平行空态、定宽容器或固定列数。拓扑可视化、静态统计卡和聊天消息流不属于卡片列表。
