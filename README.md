...
- 媒体治理任务列表以共享 Tabs 切换表格/看板，两种视图都保持满高并使用 antdv-next 标准 Empty；支持真实新建、详情查看、下载前身份编辑，以及带 revision 门的空草稿删除。表格行与作品标题不隐式打开详情，统一“查看/编辑/删除空草稿”行操作承接 CRUD；操作触发器必须阻止冒泡，不能误触行事件。
