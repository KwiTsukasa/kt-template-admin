...
- 系统管理 / 网络管理使用 TSX、KtTable 与统一 Vben 表单维护 API 持久化的逻辑端口转发组；每组只配置一对内外端口，支持 TCP、UDP、TCP+UDP，并在同一行分别展示 TCP 静态转发与 NATMap、UDP 静态转发与 Keeper 的期望/实际状态，缺失通道显示 `—`。两个协议通道可独立重试、启停机制、复制公网端点和查看按协议区分的端点历史；结构字段在任一机制启用或协调中时锁定，名称与备注仍可编辑。全部通道已同步、NATMap/Keeper 已停用且公网端点已撤回时，删除作为第二个直显操作调用现有逻辑组删除接口。DDNS 页签可将 A 记录绑定到 TCP NATMap 或 UDP Keeper 通道，分别展示原始公网端点、DNS 地址和派生的 `FQDN:端口`；AAAA 可选择 Agent 全局 IPv6，或选择 TCP NATMap IP4P 来源以编码公网 IPv4 与动态端口。首屏读取一次 HTTP 快照，后续只按唯一 API SSE 的语义资源来源刷新当前活动页签；心跳、相同端点续租和其他页签事件不刷新，不使用定时轮询。网络页显式保留两个直显行操作，其余收进更多操作。`Page autoContentHeight` 中的 Tabs 与 KtTable 必须由满高纵向 flex 外壳承接，活动面板保持 `flex-1 min-h-0`，否则 KtTable 的百分比高度链会塌陷。Admin 不接触路由器、MQTT 或腾讯云凭据。
- 顶级“媒体治理”以“系列资料库”为默认入口，唯一层级为 `Series → Work → Season/Episode → Task`。新 Series 必须先搜索并选择官方主身份，创建时原子建立主 Work；TV、电影和剧场版都作为 Series 下的 Work，只有 TV Work 可创建 Season/Episode，电影与剧场版不伪造 S00。系列列表固定使用 KtTable 筛选/分页和卡片 footer，不提供表格/看板切换；卡片操作栏只显示带 Tooltip 与 `aria-label` 的语义图标。独立 `Media:Governance:Delete` 权限只对 Season/Episode/Task/绑定/RSS 全为 0 的空壳显示删除图标，确认请求携卡片 revision，服务端 409 拒绝过期或非空删除，SSE 删除墓碑原位移除卡片。系列详情占满页面剩余高度，TV Work 的各季与电影/剧场版 Work 展平为同一排 Antdv card Tabs 上下文，不再堆叠 Work/Season 两排导航或自绘按钮卡；Episode 使用普通 KtTable 加服务端分页，并分别提供 RSS 订阅与执行历史 Tab。
- Task 页面是纯执行投影，不提供单独新建或身份编辑。新 Task 只能从系列详情的既有 Work、逐集磁链或 RSS 创建，标题、资料身份、年份、`seriesId/workId/operationKind` 均由 Work 派生。普通 TV Task 在资料身份、来源清单与 Season/Episode 视频映射全部验证后，只绑定目标 Work 已存在的 Episode；跨 Work、缺季、缺集或占用冲突零写。系列资料库复用 `/media-governance/events/stream` 接收提交后的 `catalog-changed`，当前页已有卡片原位替换，新 Series、筛选边界变化或 `snapshot-required` 静默重载分页。
- Admin 动态卡片列表统一由无业务依赖的 `KtCardList` 承担摘要 slot、Loading/Empty、全宽响应式网格和间距；`default/compact` 只规定最小可读密度，不规定卡片最大宽度或每排数量。系列资料库、媒体执行任务和大模型配置看板以 `minmax(..., 1fr)` 随实时可用宽度自动伸缩，`KtCardListCard` 统一媒体治理同款图标操作栏；Season 导航、Episode 表格、拓扑、静态统计和聊天消息流不套用卡片列表。
- 系列详情的批量磁链弹窗使用 1–16 条显式结构化行，每行独立填写集号和一条磁链，并以语义图标新增下一集或删除当前行；集号按当前 Work Season 的 `episodeStart..episodeStart+episodeCount-1` 校验。RSS 弹窗使用三阶段 Steps：只在当前阶段显示身份搜索、发布组聚合或订阅参数，可返回已解锁阶段；身份候选必须使用 Antdv Card 的 root/body 语义结构承载点击与封面正文网格，禁止用 Button 模拟卡片，样式只允许锚定 `media-rss-discovery` 命名空间。第二阶段提供图标刷新并展示子组 RSS 的真实命中数、最近时间和样例，第三阶段仅锁定来源地址，其余订阅参数可编辑；全部季级请求强制携带 `seriesId + workId + seasonNumber`。已创建订阅使用全宽 `KtCardListCard`，操作只进入共享底部 action footer。详情页收到 `catalog-changed` 时自动回读当前 Work 概览、引用、绑定和活动 Episode 页。执行任务列表保留详情、Agent 与 revision 删除等执行合同；逐文件映射继续对大来源使用 KtTable 原生虚拟表格。任务详情仍提供来源、映射、字幕、元数据、CodexAgent、运行和证据页签，正式下载、治理与验收由数据库 Outbox 和 NAS 执行器承接。
- RSS 身份候选和来源聚合使用 30 秒专用请求窗口，覆盖服务端身份核验与九个固定来源聚合的两个连续有界阶段；其他 Admin API 继续使用全局 10 秒超时。
- Admin 有状态业务编辑器统一使用 `useVbenModal`，存在字段时同时使用 `useVbenForm`；打开流程必须先 `setData(...).open()`，再在 `onOpenChange(true)` 后重置已挂载表单。直接声明 Antdv `Modal/Form/FormItem` 的生产代码由项目级契约测试拒绝，`Modal.confirm` 仅用于一次性确认；头像裁剪等无字段画布工作区只统一 Modal。
...
- 媒体治理普通进度事件携带 `runId/runSequence` 与紧凑 Task patch，列表、看板和详情只按字段原位合并，不重新请求整页数据，也不为每个 SSE tick 展示 Spin。仅在序号缺口或 `snapshot-required` 时静默读取权威快照。qBittorrent 状态每秒轮询，进度最长 30 秒发布一次且已验证字节增长或完成时立即发布；磁链清单检查仍保持每 5 秒、最长 120 秒的独立反馈合同。
