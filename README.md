...
KT 自有组件目录统一使用 kebab-case：操作组、表格和富文本分别位于 `apps/web-antdv-next/src/components/kt-action-group`、`kt-table`、`rich-text`；QQBot 发送日志位于 `apps/web-antdv-next/src/views/qqbot/send-log`。模板与组件演示页、Vben Project/About 路由及其专用语言包不得重新进入生产目录；个人资料使用独立 `profile` 路由模块。
